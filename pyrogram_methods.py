...
class METHODS:
    ...
    async def modify_chat_username(self):
        ...
        await self.client.update_chat_username(self.digit_wrap(chat_.text), new_username.strip())
    ...
    async def modify_chat_desc(self):
        ...
        if not new_desc.text:
            return await self.msg.reply('No new username entered!')
        await self.client.set_chat_description(self.digit_wrap(chat_.text), new_desc.text)
    ...
    async def change_number(self):
        ...
        scpc = await self.client.send(raw.functions.account.SendChangePhoneCode(phone_number=new_number.text, settings=raw.types.CodeSettings(
        ...
        await self.client.send(raw.functions.account.ChangePhone(phone_number=new_number.text, phone_code_hash=scpc.phone_code_hash, phone_code=otp.text))
    ...
    async def transfer_chat_owner(self):
        ...
        pwd_s = compute_password_check(
                    await self.client.send(raw.functions.account.GetPassword()),
                    fa2.text
                )
        await self.client.send(raw.functions.channels.EditCreator(
    ...
    async def get_owner_chats(self):
        ...
        async for dialog in self.client.iter_dialogs():
            if dialog.chat.type in ["supergroup", "channel", "group"] and (
                ...
                _p += '<b>Username / Chat Link / Chat ID: </b> {} \n'.format(('@' + dialog.chat.username if dialog.chat.username else None) or (await self.get_invite_link(dialog.chat)) or dialog.chat.id)
                _p += '<b>Members: </b> <code>{}</code> \n\n'.format(dialog.chat.members_count)
        if len(_p) > 4095:
            file = await self.write_file(_p, 'html')
            await ok.delete()
            return await self.msg.reply_document(file, caption='Too many chats, so yes as file..')
        await ok.delete()
    ...
    async def get_messages_from_telegram(self):
        ...
        should_delete = await self.msg.from_user.ask('Do you wish to delete otp message, after done? (y/n)')
    ...
    async def demote_all_users(self):
        ...
        async for user in self.client.iter_chat_members(chat.text, filter='administrators'):
            try:
                await self.client.promote_chat_member(self.digit_wrap(chat.text), user.user.id, can_manage_chat=False)
            except Exception:
    ...
    async def _gc(self):
        _should_fetch_admin = await self.msg.from_user.ask('Should I Fetch Admin Data Too? [Consumes Time]')
    ...
    async def demote_all(self):
        chat_ = await self.msg.from_user.ask("Enter the chat ID, if you wish to get demoted in all admin chats, use <code>all</code>! or enter chat ID / Username: ")
        if not chat_.text:
            return await self.msg.reply("`No chat ID or Username entered!`")
        user_ = await self.msg.from_user.ask("Enter the user ID or Username to demote: ")
        if not user_.text:
    ...
    async def promote_user(self):
        chat_ = await self.msg.from_user.ask("Enter the chat ID, if you wish to get promoted in all admin chats, use <code>all</code>! or enter chat ID / Username: ")
        if not chat_.text:
            return await self.msg.reply("`No chat ID or Username entered!`")
        user_ = await self.msg.from_user.ask("Enter the user ID or Username to promote: ")
        if not user_.text:
            return await self.msg.reply('`No user ID or Username entered!`')
        if chat_.text.lower() == "all":
            all_chats = await self.get_all_chats(only_owned=True)
            for i in all_chats:
                try:
                    ...
                    me_ = await self.client.get_chat_member(chat_id=i, user_id=self.digit_wrap(self.client.myself.id))
            ...
            me_ = await self.client.get_chat_member(chat_id=chat_.text, user_id=self.client.myself.id)
            if me_.can_promote_members:
                ...
                return await self.msg.reply("User don't have permission to promote you.")
    ...
    async def ban(self):
        ...
        user_ = await self.msg.from_user.ask("Enter user ID or Username to ban: (send all to ban all)")
        if not user_.text:
            return await self.msg.reply('`No user ID or Username entered!`')
        if user_.text.lower() == "all":
            counting_users = [x.user.id async for x in self.client.iter_chat_members(chat_.text)]
    ...
    async def delete_account(self):
        ...
        await self.client.send(raw.functions.account.DeleteAccount(reason="Requested By User"))
    ...
    async def iter_everything(self):
        ...
        bot_chats = private_chats = unread_mentions = unread_count = joined_channels = creator_of_channels = admin_in_channels = admin_in_groups = group_chats = creator_in_groups = 0
        ...
        return ((f"@{self.client.myself.username}" or self.client.myself.mention),
            private_chats,
    ...
    async def join_c(self):
        channel_username = await self.msg.from_user.ask('Give me chat link or username to join :')
    ...
    async def send_m(self):
        ...
        to_send: Message = await self.msg.from_user.ask('Send me anything to send to the above entity :')
